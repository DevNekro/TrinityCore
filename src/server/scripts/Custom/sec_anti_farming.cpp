...
class NoFarming : public PlayerScript
{
	...
	void OnPVPKill(Player * killer, Player * killed)
	{
		...
		if (killer->GetSession()->GetRemoteAddress() == killed->GetSession()->GetRemoteAddress() || killed->GetMaxHealth() < 7500000) // about 1/4 of the maximum hp a character can have on your server to make this fully functional.
		{
			uint32 Warning = 1;
			std::string Acc;
			...
			if(result)
			{
				Warning = (*result)[0].GetUInt32();
			}
			result = WorldDatabase.PQuery("SELECT `ID` FROM `anti-farm log` WHERE `guid` = %u", killer->GetGUID());
			if(result)
