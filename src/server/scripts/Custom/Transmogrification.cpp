    ...
    */
    ...
    else
    {
        ...
        if (!no_cost)
        {
            ...
            if (GetRequireToken())
            {
				if (player->HasItemCount(GetTokenEntry(itemTransmogrified->GetTemplate()->InventoryType), GetTokenAmount(itemTransmogrified->GetTemplate()->InventoryType)))
                    player->DestroyItemCount(GetTokenEntry(itemTransmogrified->GetTemplate()->InventoryType), GetTokenAmount(itemTransmogrified->GetTemplate()->InventoryType), true);
                else
...
bool Transmogrification::CanTransmogrifyItemWithItem(Player* player, ItemTemplate const* proto2, ItemTemplate const* proto1)
{
	...
	uint32 newClass = proto1->Class;
	uint32 oldClass = proto2->Class;
	uint32 newSubClass = proto1->SubClass;
	uint32 oldSubClass = proto2->SubClass;
	uint32 newInventorytype = proto1->InventoryType;
	uint32 oldInventorytype = proto2->InventoryType;
    ...
    {
		if(((newSubClass == 0 || newSubClass == 1 || newSubClass == 4 || newSubClass == 5 || newSubClass == 6 || newSubClass == 7 || newSubClass == 8 || newSubClass == 10 || newSubClass == 13 || newSubClass == 15 || newSubClass == 17)
			&& (oldSubClass == 0 || oldSubClass == 1 || oldSubClass == 4 || oldSubClass == 5 || oldSubClass == 6 || oldSubClass == 7 || oldSubClass == 8 || oldSubClass == 10 || oldSubClass == 13 || oldSubClass == 15 || oldSubClass == 17))
			|| ((newSubClass == 2 || newSubClass == 3 || newSubClass == 16 || newSubClass == 18 || newSubClass == 19) && (oldSubClass == 2 || oldSubClass == 3 || oldSubClass == 16 || oldSubClass == 18 || oldSubClass == 19)))
        ...
        //if (newSubClass == oldSubClass) Allows cloth -> plate and etc
...
uint32 Transmogrification::GetTokenEntry(uint32 inventoryType) const
{
    uint32 entry = entries[inventoryType];
		return entry ? entry : TokenEntry;
}
uint32 Transmogrification::GetTokenAmount(uint32 inventoryType) const
{
...
void Transmogrification::LoadConfig(bool reload)
{
	...
	for(uint32 i = 0; i < 30; ++i)
        ...
        entries[INVTYPE_BODY] = (uint32)sConfigMgr->GetIntDefault("Transmogrification.BodyTokenEntry", 49426);
        amounts[INVTYPE_BODY] = (uint32)sConfigMgr->GetIntDefault("Transmogrification.BodyTokenAmount", 1);
